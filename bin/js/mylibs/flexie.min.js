var Flexie=function(x,E){function F(a){if(a)a=a.replace(wa,v).replace(xa,v);return a}function h(a,d){var c,b=0,e=a?a.length:S;if(e===S)for(c in a){if(a.hasOwnProperty(c))if(d.call(a[c],c,a[c])===I)break}else for(c=a[0];b<e&&d.call(c,b,c)!==I;c=a[++b]);}function ya(){var a;h(X,function(d,c){if(x[d]&&!a)if(a=eval(c.s.replace("*",d))){Y=d;return false}});return a}function Z(a,d){a="on"+a;var c=x[a];x[a]=typeof x[a]!=="function"?d:function(){c&&c();d()}}function $(a){var d=a.nodeName.toLowerCase();if(a.id)d+=
"#"+a.id;else if(a.FLX_DOM_ID)d+="["+aa+"='"+a.FLX_DOM_ID+"']";return d}function T(a){if(!a.FLX_DOM_ID){U+=1;a.FLX_DOM_ID=U;a.setAttribute(aa,a.FLX_DOM_ID)}}function za(a){var d=[],c,b,e,f,g,i;a=a.replace(Aa,v);a=a.replace(/\s?(\{|\:|\})\s?/g,P);a=a.split(ba);h(a,function(j,l){if(l){c=[l,ba].join(v);if((b=/(.*)\{(.*)\}/.exec(c))&&b[2]){e=b[1];f=b[2].split(";");i=[];h(f,function(m,r){g=r.split(":");g.length&&g[1]&&i.push({property:g[0],value:g[1]})});e&&i.length&&d.push({selector:e,properties:i})}}});
...
function(g,i){if(i.selector){b=d(i.selector);b=b[0]?b:[b];b[0]&&h(b,function(j,l){if(l.nodeName!==S)switch(l.nodeName.toLowerCase()){case "script":case "style":case "link":break;default:if(l.parentNode===a){T(l);e={};h(i,function(m){e[m]=i[m]});e.match=l;f.push(e)}}})}else{T(i);f.push({match:i,selector:$(i)})}});return f}function Ca(a){h(a,function(d,c){a[d]=c||Da[d]});return a}function Ea(a){var d,c,b,e,f,g,i,j,l,m,r,n,o,k,p={},s,w,B,M="["+Q+"]";if(a){h(a.boxes,function(A,C){C.selector=F(C.selector);
d=C.selector;c=C.properties;g=i=j=l=m=t;h(c,function(Fa,G){b=F(G.property);e=F(G.value);if(b){f=b.replace("box-",v);switch(f){case "display":if(e==="box")g=e;break;case "orient":i=e;break;case "align":j=e;break;case "direction":l=e;break;case "pack":m=e}}});r=J;n=r(C.selector);n=n[0]?n:[n];h(n,function(Fa,G){if(G.nodeType){T(G);o=da(G,r,a.children);B=d+" "+M;k={target:G,selector:d,properties:c,children:o,display:g,orient:i,align:j,direction:l,pack:m,nested:B};if(s=p[G.FLX_DOM_ID])h(k,function(K,N){switch(K){case "selector":if(N&&
...
[ja,ka,la,ma];b=ga(a,b,d);break;case "height":b=[V,na,oa,pa];b=ga(a,b,d);break;default:b=ha(a,d)}a=b}else a=ha(a,d);b=a}a=b}else a=a.currentStyle[d];if(c){a=parseInt(a,10);if(isNaN(a))a=0}return a}function Ja(a){return a.innerWidth||a.clientWidth}function Ka(a){return a.innerHeight||a.clientHeight}function W(a,d,c,b){var e=[];h(La,function(f,g){e.push((b?g:v)+d+":"+(!b?g:v)+c)});a.style.cssText+=e.join(";");return a}function D(a,d,c){h(a&&a[0]?a:[a],function(b,e){if(e&&e.style)e.style[d]=c?c+"px":
...
a.target;if(!c.FLX_DOM_ID)c.FLX_DOM_ID=c.FLX_DOM_ID||++U;if(!a.nodes)a.nodes=O(c,c.childNodes);if(!a.selector){a.selector=$(c);c.setAttribute(Q,q)}if(!a.properties)a.properties=[];if(!a.children)a.children=da(c,J,O(c,c.childNodes));if(!a.nested)a.nested=a.selector+" ["+Q+"]";a.target=c;a._instance=d;return a}var z={},U=0,aa="data-flexie-id",Q="data-flexie-parent",H,Y,X={NW:{s:"*.Dom.select"},DOMAssistant:{s:"*.$",m:"*.DOMReady"},Prototype:{s:"$$",m:"document.observe",p:"dom:loaded",c:"document"},
...
na="paddingBottom",ja="paddingLeft",V="paddingTop",ma="borderRightWidth",pa="borderBottomWidth",la="borderLeftWidth",oa="borderTopWidth",ba="}",La=" -o- -moz- -ms- -webkit- -khtml- ".split(ua),Da={orient:"horizontal",align:"stretch",direction:"inherit",pack:"start"},y=[],ca=[],fa,ra,q=true,I=false,t=null,S,L={IE:function(){var a,d=Ua.exec(x.navigator.userAgent.toLowerCase());if(d)a=parseInt(d[2],10);return a}()},va;va=function(){function a(o){return o.replace(j,function(k,p,s){k=s.split(",");h(k,
...
func:Ka,pad:[V,na,oa,pa]};H||h(d,function(e,f){f.style[L.IE>=9?"cssFloat":"styleFloat"]="left";if(c.orient==="vertical"||c.orient==="block-axis")f.style.clear="left";if(L.IE===6)f.style.display="inline"});switch(c.orient){case "vertical":case "block-axis":this.props=b;this.anti=a;break;default:this.props=a;this.anti=b}},boxOrdinalGroup:function(a,d,c){var b,e;if(d.length){b=function(f){f=f.keys;h(c.reversed?f:f.reverse(),function(g,i){h(d,function(j,l){i===l.getAttribute("data-ordinal-group")&&a.appendChild(l)})})};
...
W(c,"display","box",t);W(c,"box-align","stretch",q);W(c,"box-pack","justify",q);E.body.appendChild(c);d=c.firstChild.offsetHeight;h({boxAlignStretch:function(){return d===100},boxPackJustify:function(){var e=0;h(c.childNodes,function(f,g){e+=g.offsetLeft});return e===135}},function(e,f){b=f();if(!b)a.partialSupport=q;a[e]=b});E.body.removeChild(c);return~c.style.display.indexOf("box")?a:I};z.init=function(){z.flexboxSupported=H=z.flexboxSupport();if((!H||H.partialSupport)&&J)va()};z.version="1.0.1";
(function(a){Y||(J=ya());var d,c,b;h(X,function(e,f){if(x[e]&&!d&&f.m){d=eval(f.m.replace("*",e));c=f.c?eval(f.c.replace("*",e)):x;b=[];if(d&&c){f.p&&b.push(f.p);b.push(a);d.apply(c,b);return false}}});d||Z("load",a)})(z.init);return z}(this,document);